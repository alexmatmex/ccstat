...
def get_queues_info():
    pbsnodes = subprocess.check_output( athost + ["/mnt/rhel-6/huawei/torque/6.0.0/bin/qstat @pbs-ds -Q -f"], shell=True)
...
def get_nodes_info():
    pbsnodes = subprocess.check_output(athost + ["/mnt/rhel-6/huawei/torque/6.0.0/bin/pbsnodes -s pbs-ds -a"], shell=True)
    lines = pbsnodes.replace("\n\t","").split("\n")
    i = 0
    nodes = {}
    while (i < len(lines)):
...
def running_jobs_info():
    pbsnodes = subprocess.check_output( athost + [ "/mnt/rhel-6/huawei/torque/6.0.0/bin/qstat @pbs-ds -f"], shell=True)
